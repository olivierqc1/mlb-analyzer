...
#!/usr/bin/env python3
"""
MLB Betting Analyzer v5.0
Marchés actifs : pitcher_strikeouts, pitcher_hits_allowed, batter_total_bases
Retirés : pitcher_earned_runs (variance défense/bullpen), batter_hits (distribution binaire 0.5 inexploitable)
Nouveau : score de confiance BET QUALITY (A/B/C/AVOID) visible dans la réponse
"""
...
STAT_CONFIG = {
    'pitcher_strikeouts': {
        ...
        'why_good': 'Distribution normale stable. Stat individuelle du lanceur, peu de bruit externe.',
    },
    'pitcher_hits_allowed': {
        'group': 'pitching', 'col': 'hits',
        'label': 'HA', 'min_ip': 3.0, 'dist': 'normal',
        'bettable': True, 'min_games': 5,
        'why_good': 'Corrélé au WHIP du lanceur. Moins de variance que ER. Normale acceptable sur saison complète.',
    },
    'batter_total_bases': {
        'group': 'hitting', 'col': 'totalBases',
        'label': 'TB', 'min_ip': None, 'dist': 'normal',
        'bettable': True, 'min_games': 10,
        'why_good': 'Valeurs discrètes mais assez élevées (0-4+) pour approcher la normale. Meilleur que Hits.',
    },
    # Retirés — jamais scannés mais gardés pour documentation
    'pitcher_earned_runs': {
        'group': 'pitching', 'col': 'earnedRuns',
        'label': 'ER', 'min_ip': 3.0, 'dist': 'poisson',
        'bettable': False, 'min_games': 5,
        'why_good': None,
        'why_removed': 'Dépend de la défense et du bullpen. Variance inexploitable. Consistance chroniquement <30%.',
    },
    'batter_hits': {
        'group': 'hitting', 'col': 'hits',
        'label': 'H', 'min_ip': None, 'dist': 'normal',
        'bettable': False, 'min_games': 10,
        'why_good': None,
        'why_removed': 'Lignes à 0.5 H = distribution Bernoulli, pas normale. Trop de faux signaux. 106 résultats "douteux" en test.',
    },
}

BETTABLE_STATS = [k for k,v in STAT_CONFIG.items() if v['bettable']]

# ── Bet Quality Score ─────────────────────────────────────────────────────────
def bet_quality(a, cfg):
    """
    Score de confiance combiné. Retourne grade A/B/C/AVOID + raisons.
    """
    score  = 0
    issues = []
    pros   = []
    ...
    # 1. Nombre de matchs
    if a['n'] >= 15:
        score += 2; pros.append(f"✅ {a['n']} matchs analysés (solide)")
    elif a['n'] >= 8:
        score += 1; pros.append(f"🟡 {a['n']} matchs (acceptable)")
    else:
        issues.append(f"❌ Seulement {a['n']} matchs — données insuffisantes")

    # 2. Consistance
    if a['cons'] >= 70:
        score += 2; pros.append(f"✅ Consistance {a['cons']}% (très stable)")
    elif a['cons'] >= 50:
        score += 1; pros.append(f"🟡 Consistance {a['cons']}%")
    else:
        issues.append(f"❌ Consistance {a['cons']}% — lanceur imprévisible")

    # 3. Distribution
    verdict = a['normality']['verdict']
    if verdict == 'NORMAL':
        score += 2; pros.append("✅ Distribution normale confirmée (tests SW+KS)")
    elif verdict == 'BORDERLINE':
        score += 1; pros.append("🟡 Distribution approximativement normale")
    else:
        issues.append("❌ Distribution non-normale — edge peu fiable")

    # 4. Chi² goodness of fit
    gof = a.get('chi_gof')
    if gof and gof['is_good_fit']:
        score += 1; pros.append("✅ Chi² GOF : fréquences OVER/UNDER cohérentes")
    elif gof and not gof['is_good_fit']:
        issues.append("⚠️ Chi² : fréquences observées ≠ théoriques")

    # 5. Marge vs ligne (signal fort = grande marge)
    margin = abs(a['adj_mean'] - a['line'])
    sigma  = a['cstd'] if a['cstd'] > 0 else 1
    z_margin = margin / sigma
    if z_margin >= 1.5:
        score += 2; pros.append(f"✅ Marge {margin:.1f} = {z_margin:.1f}σ (signal fort)")
    elif z_margin >= 0.8:
        score += 1; pros.append(f"🟡 Marge {margin:.1f} = {z_margin:.1f}σ")
    else:
        issues.append(f"❌ Marge trop faible ({margin:.1f}) — ligne trop proche de la moyenne")

    # 6. L5 cohérent avec recommandation
    rec = a['rec']
    l5  = a['l5']
    line = a['line']
    if rec == 'OVER' and l5 > line:
        score += 1; pros.append(f"✅ L5 ({l5}) confirme OVER la ligne ({line})")
    elif rec == 'UNDER' and l5 < line:
        score += 1; pros.append(f"✅ L5 ({l5}) confirme UNDER la ligne ({line})")
    elif rec == 'OVER' and l5 <= line:
        issues.append(f"⚠️ L5 ({l5}) contredit le OVER — forme récente douteuse")
    elif rec == 'UNDER' and l5 >= line:
        issues.append(f"⚠️ L5 ({l5}) contredit le UNDER — forme récente douteuse")

    # 7. TB spécifique: moyenne trop basse = Bernoulli territory
    if cfg['label'] == 'TB' and a['cmean'] < 1.0:
        issues.append("❌ Moyenne TB < 1.0 — trop peu de bases, distribution inadaptée")
        score -= 2
    ...
    if score >= 8 and not issues:
        ...
        grade, color, label = 'AVOID', '#f87171', '🔴 ÉVITER — Trop de problèmes'
    ...
    try:
        ...
        for s in splits:
            ...
            try:
                ...
                ip = 0.0
            if cfg['min_ip'] and ip < cfg['min_ip']: continue
            (home_vals if s.get('isHome', True) else away_vals).append(int(val))
        result = {
            ...
            'home_n': len(home_vals), 'away_n': len(away_vals),
        }
...
def get_rest_days(games):
    ...
    try:
        ...
        return min((datetime.now() - last).days, 15)
    except:
        return None
...
def get_gamelog(player_id, stat_type):
    ...
    try:
        ...
        print(f"Gamelog error {player_id} {stat_type}: {e}"); return None
...
def normality_tests(vals):
    ...
    if sw_ok and ks_ok:     verdict, label, penalty = 'NORMAL',     '✅ Distribution normale confirmée',         0
    elif sw_ok or ks_ok:    verdict, label, penalty = 'BORDERLINE', '⚠️ Distribution approximativement normale', 10
    else:                   verdict, label, penalty = 'NON_NORMAL', '❌ Non-normale — edge pénalisé',            25
...
def chi_gof(vals, line):
    ...
    try:
        ...
        over_exp  = p_over * n; under_exp = (1 - p_over) * n
        if over_exp < 1 or under_exp < 1: return None
        ...
        return {'chi2': round(float(chi2),4), 'p_value': round(float(p),4),
                ...
                'observed': [over_obs, under_obs], 'expected': [round(over_exp,1), round(under_exp,1)]}
    except: return None

def compute_context_adjustment(stat_type, cmean, opponent, is_home, splits, rest_days):
    adjustments = {}; total_adj = 0.0
    if stat_type in ('pitcher_strikeouts', 'pitcher_hits_allowed'):
        opp_k_pct = get_opponent_k_pct(opponent) if opponent else None
        if opp_k_pct is not None:
            diff = opp_k_pct - LEAGUE_AVG_K_PCT
            adj  = round(diff / 0.01 * (0.3 if stat_type=='pitcher_strikeouts' else -0.2), 2)
            adjustments['opponent_k_pct'] = {
                'value': round(opp_k_pct*100,1), 'league_avg': round(LEAGUE_AVG_K_PCT*100,1),
                'adjustment': adj, 'label': f"Lineup K% {round(opp_k_pct*100,1)}% vs moy {round(LEAGUE_AVG_K_PCT*100,1)}%"
            }
            total_adj += adj
        else:
            adjustments['opponent_k_pct'] = {'value': None, 'adjustment': 0.0, 'label': 'K% N/A'}
        if splits:
            hm, am = splits.get('home_mean'), splits.get('away_mean')
            if hm is not None and am is not None:
                loc_mean = hm if is_home else am
                adj = round(max(-2.0, min(2.0, loc_mean - cmean)), 2)
                adjustments['home_away_split'] = {
                    'is_home': is_home, 'home_mean': hm, 'away_mean': am, 'adjustment': adj,
                    'label': f"{'Dom' if is_home else 'Ext'}: moy {loc_mean} (global {cmean})"
                }
                total_adj += adj
            else:
        ...
        if rest_days is not None:
            if rest_days <= 2:    adj, lbl = -0.5, f"Court repos ({rest_days}j)"
            elif rest_days >= 6:  adj, lbl = -0.3, f"Long repos ({rest_days}j)"
            else:                 adj, lbl =  0.2, f"Repos normal ({rest_days}j)"
            adjustments['rest_days'] = {'days': rest_days, 'adjustment': adj, 'label': lbl}
            total_adj += adj
        else:
            adjustments['rest_days'] = {'days': None, 'adjustment': 0.0, 'label': 'Inconnu'}
    else:
        adjustments = {
            'opponent_k_pct':  {'adjustment': 0.0, 'label': 'N/A'},
            'home_away_split': {'adjustment': 0.0, 'label': 'N/A'},
            'rest_days':       {'adjustment': 0.0, 'label': 'N/A'}
        }
    return round(cmean + total_adj, 2), round(total_adj, 2), adjustments
...
def analyze(games, line, stat_type, opponent=None, is_home=True, player_id=None):
    ...
    if len(qualified) < 4: qualified = games
    if len(qualified) < 4: return None
    ...
    # Build intermediate dict for bet_quality
    a_dict = {
        ...
        'normality': norm_res, 'chi_gof': gof, 'dist_used': 'normal',
        'line': line,
        'context': {'adjustments': adjustments, 'rest_days': rest_days,
                    'is_home': is_home, 'opponent': opponent or ''},
        'recent': recent
    }
    a_dict['quality'] = bet_quality(a_dict, cfg)
    return a_dict

def get_all_props(stat_type_filter=None):
    if not ODDS_API_KEY: return {}, {}
    markets = [stat_type_filter] if stat_type_filter else BETTABLE_STATS
    try:
        ...
        try:
            ...
            for bk in resp2.json().get('bookmakers',[]):
                for mk in bk.get('markets',[]):
                    mkey = mk['key']
                    if mkey not in all_props: continue
...
def scan(stat_type_filter=None, min_edge=5.0):
    ...
    stat_types = [stat_type_filter] if stat_type_filter else BETTABLE_STATS
    ...
    for stat_type in stat_types:
        ...
        for prop_name, prop_data in props.items():
            ...
            # Skip AVOID grades — not worth showing
            ...
            opportunities.append({
                ...
                'game_info': {'home_team': gi.get('home_team',''), 'away_team': gi.get('away_team',''),
                              'time': gi.get('time',''), 'is_home': is_home, 'opponent': opponent},
                'quality': a['quality'],
                'line_analysis': {
                    'bookmaker_line': line, 'bookmaker': best['book'].upper(),
                    'recommendation': a['rec'], 'raw_edge': a['raw_edge'],
                    'edge': a['edge'], 'over_probability': a['over_p'],
                    'under_probability': a['under_p'], 'kelly_criterion': a['kelly']
                },
                'context': {
                    'opponent': opponent, 'is_home': is_home,
                    'rest_days': a['context']['rest_days'],
                    'base_mean': a['cmean'], 'adjusted_mean': a['adj_mean'],
    ...
    # Sort: A first, then B, then C; within grade by edge
    grade_order = {'A': 0, 'B': 1, 'C': 2, 'AVOID': 3}
    opportunities.sort(key=lambda x: (
        grade_order.get(x['quality']['grade'], 3),
    ...
    try:
        ...
        if stat_type and stat_type not in STAT_CONFIG:
            return jsonify({'status':'ERROR',
                'message': f"stat_type doit être: {list(STAT_CONFIG.keys())}"}), 400
        opps, analyzed, n_pitchers = scan(stat_type, min_edge)
        return jsonify(to_python({
            'status': 'SUCCESS', 'sport': 'MLB', 'version': '5.0',
            'active_markets': BETTABLE_STATS,
            'removed_markets': {
                'pitcher_earned_runs': 'Variance défense/bullpen inexploitable',
                'batter_hits': 'Distribution Bernoulli sur lignes 0.5 — faux signaux'
            },
...
def markets():
    return jsonify({'active': [
        {'key': k, 'label': v['label'], 'why_good': v['why_good']}
        for k,v in STAT_CONFIG.items() if v['bettable']
    ], 'removed': [
        {'key': k, 'label': v['label'], 'why_removed': v.get('why_removed')}
        for k,v in STAT_CONFIG.items() if not v['bettable']
    ]})
...
def health():
    return jsonify({'status':'healthy','version':'5.0','sport':'MLB',
                    'active_markets': BETTABLE_STATS,
                    'quality_grades': ['A','B','C'],
                    'removed': ['pitcher_earned_runs','batter_hits']})

@app.route('/')
def home():
    return jsonify({'app':'MLB Betting Analyzer','version':'5.0'})
